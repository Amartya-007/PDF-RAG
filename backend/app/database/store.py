...
DEFAULT_SESSION_ID: Final = "default"
DEFAULT_SESSION_TITLE: Final = "Welcome"

# --- Pre-allocated SQL Constants ---
# Compiling these at the module level prevents massive string allocations 
# in Python's memory every time the database initializes or writes.
_INIT_SCHEMA_SQL: Final = """
...
class MetadataStore:
    ...
    def mark_stale_processing_documents_failed(self) -> int:
        with self.connect() as conn:
            rows = conn.execute(_MARK_STALE_FAILED_SQL).fetchall()
            
            # Optimization: Generator expression inside executemany is memory efficient
            conn.executemany(
    ...
    def replace_chunks(self, document_id: str, chunks: list[Chunk]) -> None:
        with self.connect() as conn:
            ...
            # Optimization: Generator expression prevents Python from duplicating 
            # the entire chunks list in memory while parsing it for the DB bindings.
            conn.executemany(
                _INSERT_CHUNKS_SQL,
                (
                    (
    ...
    def _document_from_row(row: sqlite3.Row) -> Document:
        # Optimization: Try-except is natively handled in C by SQLite
        # Using `.keys()` dynamically allocates a full list for every parsed row.
        try:
            session_id = row["session_id"]
        except IndexError:
